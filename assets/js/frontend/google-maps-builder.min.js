...
var a=new google.maps.InfoWindow({maxWidth:315}),t=$(e).data("map-id"),o=gmb_data[t],n=o.map_params.latitude?o.map_params.latitude:"32.713240",i=o.map_params.longitude?o.map_params.longitude:"-117.159443",r={center:new google.maps.LatLng(n,i),zoom:parseInt(o.map_params.zoom),styles:[{stylers:[{visibility:"simplified"}]},{elementType:"labels",stylers:[{visibility:"off"}]}]};map=new google.maps.Map(e[0],r),places_service=new google.maps.places.PlacesService(map),set_map_options(map,o),set_map_theme(map,o),set_map_markers(map,o,a),set_map_directions(map,o),set_map_layers(map,o),set_map_places_search(map,o),
//Display places?
...
var s=a.map_controls.draggable;e.setOptions("none"===s?{draggable:!1}:{draggable:!0})}/**
	 * Set Map Markers
	 */
function set_map_markers(map,map_data,info_window){var map_markers=map_data.map_markers,markers=[];
//Cluster?
if(
...
//Add Proper Params
marker_args.place={location:{lat:parseFloat(marker_data.lat),lng:parseFloat(marker_data.lng)},placeId:marker_data.place_id},marker_args.attribution={source:map_data.site_name,webUrl:map_data.site_url});
//Marker for map
var location_marker=new Marker(marker_args);
//Opened by default?
if(markers.push(location_marker),location_marker.setVisible(!0),google.maps.event.addListener(location_marker,"click",function(){info_window.close(),info_window.setContent('<div id="infobubble-content" class="loading"></div>'),set_info_window_content(marker_data,info_window),info_window.open(map,location_marker)}),"undefined"!=typeof marker_data.infowindow_open&&"opened"==marker_data.infowindow_open){var info_window_opened=new google.maps.InfoWindow({maxWidth:315});set_info_window_content(marker_data,info_window_opened),info_window_opened.open(map,location_marker)}}),//end $.each()
console.log(map_data.marker_cluster),"yes"===map_data.marker_cluster)var markerCluster=new MarkerClusterer(map,markers)}/**
...
var r={key:gmb_data.api_key,location:new google.maps.LatLng(o.lat(),o.lng()),types:n,radius:a.places_api.search_radius};places_service.nearbySearch(r,function(a,o,n){var i,r=0;
//setup new markers
if(o==google.maps.places.PlacesServiceStatus.OK){
//place new markers
for(r=0;i=a[r];r++)create_search_result_marker(e,a[r],t);
...
var r=$(this).find(".cmb-repeat-row"),s=o.point[0].latitude,l=o.point[0].longitude,p=o.point[o.point.length-1].latitude,m=o.point[o.point.length-1].longitude,c=o.travel_mode.length>0?o.travel_mode:"DRIVING",_=[];r.not(":first").not(":last").each(function(e,a){var t=$(this).find(".gmb-directions-latitude").val(),o=$(this).find(".gmb-directions-longitude").val();_.push({location:t+","+o,stopover:!0})});var d={origin:s+","+l,destination:p+","+m,waypoints:_,optimizeWaypoints:!0,travelMode:google.maps.TravelMode[c]};n.route(d,function(e,a){a==google.maps.DirectionsStatus.OK&&
...
//sanity check
if("yes"!==a.places_search[0])return!1;{var o=$("#google-maps-builder-"+a.id).siblings(".places-search-wrap"),n=/** @type {HTMLInputElement} */o.find("#pac-input").get(0);$("#google-maps-builder-"+a.id).siblings(".places-search-wrap").find("#type-selector").get(0)}e.controls[google.maps.ControlPosition.TOP_CENTER].push(o.get(0));var i=new google.maps.places.Autocomplete(n);i.bindTo("bounds",e);var r=new google.maps.InfoWindow,s=new google.maps.Marker({map:e,anchorPoint:new google.maps.Point(0,-29)});google.maps.event.addListener(i,"place_changed",function(){r.close(),s.setVisible(!1);var a=i.getPlace();if(console.log(a),!a.geometry)return void window.alert("Autocomplete's returned place contains no geometry");
// If the place has a geometry, then present it on a map.
a.geometry.viewport?e.fitBounds(a.geometry.viewport):(e.setCenter(a.geometry.location),e.setZoom(17)),s.setIcon(/** @type {google.maps.Icon} */{url:a.icon,size:new google.maps.Size(71,71),origin:new google.maps.Point(0,0),anchor:new google.maps.Point(17,34),scaledSize:new google.maps.Size(35,35)}),s.setPosition(a.geometry.location),s.setVisible(!0);var t;a.name&&(t='<p class="place-title">'+a.name+"</p>"),t+=set_place_content_in_info_window(a),t=set_info_window_wrapper(t),//wraps the content in div and returns
r.setContent(t),//set marker content
r.open(e,s)}),t("changetype-all",[]),t("changetype-address",["address"]),t("changetype-establishment",["establishment"]),t("changetype-geocode",["geocode"])}var map,places_service,place,directionsDisplay=[],search_markers=[];/*
