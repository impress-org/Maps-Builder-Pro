...
var a=new google.maps.InfoWindow({maxWidth:315}),t=$(e).data("map-id"),o=gmb_data[t],n=o.map_params.latitude?o.map_params.latitude:"32.713240",i=o.map_params.longitude?o.map_params.longitude:"-117.159443",r={center:new google.maps.LatLng(n,i),zoom:parseInt(o.map_params.zoom),styles:[{stylers:[{visibility:"simplified"}]},{elementType:"labels",stylers:[{visibility:"off"}]}]};map=new google.maps.Map(e[0],r),places_service=new google.maps.places.PlacesService(map),set_map_options(map,o),set_map_theme(map,o),set_map_markers(map,o,a),set_map_directions(map,o),
//Display places?
...
//Setup destinations
$(a.destination_markers).each(function(t,o){console.log(a);var n=new google.maps.DirectionsService,i=new google.maps.DirectionsRenderer;i.setMap(e),"none"!==a.text_directions&&($("#directions-panel-"+a.id).addClass("panel-"+a.text_directions),i.setPanel($("#directions-panel-"+a.id).get(0)));
...
i.setDirections(e)})})}var map,places_service,place,directionsDisplay=[],search_markers=[];/*
