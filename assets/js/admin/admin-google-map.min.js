...
navigator.geolocation&&"yes"===gmb_data.geolocate_setting&&""==t&&""==a&&"https:"===location.protocol?navigator.geolocation.getCurrentPosition(function(e){initial_location=new google.maps.LatLng(e.coords.latitude,e.coords.longitude),map.setCenter(initial_location),//set map with location
...
set_map_type(!1),set_map_theme(!1),set_street_view(),set_pan_control(),set_draggable(),set_double_click_zoom(),set_mouse_wheel_scroll(),set_map_type_control(),set_map_zoom_control(),"undefined"==typeof autocomplete){var i=$("#gmb_geocoder");autocomplete=new google.maps.places.Autocomplete(i[0]),autocomplete.bindTo("bounds",map),
