...
"undefined"!=typeof i&&i.setVisible(!1),$(".drop-marker").removeClass("active").html(gmb_data.i18n.btn_drop_marker),//reset drop button
...
//add data to fields
$("#gmb_markers_group_"+n+"_title").val("Point "+parseInt(n+1)),//increment index to match visual ID (actually 0)
$("#gmb_markers_group_"+n+"_lat").val(t),$("#gmb_markers_group_"+n+"_lng").val(o),get_editable_info_window(n,i),google.maps.event.addListener(i,"click",function(){get_info_window_content(n,i)})}/**
	 * Map Intialize
	 *
...
navigator.geolocation&&"yes"===gmb_data.geolocate_setting&&""==i&&""==o&&"https:"===location.protocol?navigator.geolocation.getCurrentPosition(function(e){initial_location=new google.maps.LatLng(e.coords.latitude,e.coords.longitude),map.setCenter(initial_location),//set map with location
...
set_map_type(!1),set_map_theme(!1),set_street_view(),set_pan_control(),set_draggable(),set_double_click_zoom(),set_mouse_wheel_scroll(),set_map_type_control(),set_map_zoom_control(),"undefined"==typeof autocomplete){var r=$("#gmb_geocoder");autocomplete=new google.maps.places.Autocomplete(r[0]),autocomplete.bindTo("bounds",map),
...
//Directions Option
//close wrapper
return a='<div class="marker-info-wrapper">',a+=e.formatted_address?'<div class="place-address">'+e.formatted_address+"</div>":"",a+=e.formatted_phone_number?'<div class="place-phone">'+e.formatted_phone_number+"</div>":"",a+=e.website?'<div class="place-website"><a href="'+e.website+'" target="_blank" rel="nofollow" title="Click to visit the '+e.name+' website">'+gmb_data.i18n.visit_website+"</a></div>":"",e.rating&&(a+='<div class="rating-wrap clear"><p class="numeric-rating">'+e.rating+'</p><div class="star-rating-wrap"><div class="star-rating-size" style="width:'+65*e.rating/5+'px;"></div></div></div>'),e.formatted_address&&(a+='<a href="https://www.google.com/maps/dir/Current+Location/'+encodeURIComponent(e.formatted_address)+'" target="_blank" title="'+gmb_data.i18n.get_directions+'">'+gmb_data.i18n.get_directions+"</a>"),a+="</div>"}/**
	 * info_bubble Content for Place Details
	 *
	 * This marker contains more information about the place
	 */
...
var edit_marker_icon_button_click=google.maps.event.addDomListener($(".marker-edit-link-wrap")[0],"click",function(){$(".save-marker-button").attr("data-marker-index",$(this).data("index"))}),save_icon_listener=google.maps.event.addDomListener($(".save-marker-button")[0],"click",function(e){e.preventDefault();var marker_position=marker.getPosition(),marker_icon=$(this).data("marker"),marker_icon_color=$(this).data("marker-color"),label_color=$(this).data("label-color"),marker_icon_data,marker_label_inline_style="color:"+label_color+"; ";"MAP_PIN"===marker_icon?marker_label_inline_style+="font-size: 20px;position: relative; top: -3px;":"SQUARE_PIN"==marker_icon&&(marker_label_inline_style+="font-size: 20px;position: relative; top: 12px;");
...
marker.setMap(null);var marker_args={position:marker_position,map:map,zIndex:9,icon:marker_icon_data,custom_label:marker_label_data};
//Update Icon
...
0===$(".magnific-builder").length?$.magnificPopup.close():$(".gmb-modal-close").trigger("click"),google.maps.event.removeListener(save_icon_listener),//remove this event listener
google.maps.event.removeListener(edit_marker_icon_button_click)})}/**
...
hover_circle.setVisible(!0)})}/**
	 ...
	 * @param map
	 */
function add_markers(map){clear_main_markers();var time=500,markers=[],cluster_markers=$("#gmb_marker_cluster1").prop("checked");//end $.each()
//Cluster?
if(
...
//setTimeout( function () {
var marker_icon=gmb_data.default_marker,marker_label="",custom_marker_icon=$("#gmb_markers_group_"+index+"_marker").val(),custom_marker_img=$("#gmb_markers_group_"+index+"_marker_img").val();if(custom_marker_img)marker_icon=custom_marker_img;else if(custom_marker_icon.length>0&&custom_marker_icon.length>0){var custom_label=$("#gmb_markers_group_"+index+"_label").val();marker_icon=eval("("+custom_marker_icon+")"),marker_label=custom_label}var marker_lat=parseFloat($("#gmb_markers_group_"+index+"_lat").val()),marker_lng=parseFloat($("#gmb_markers_group_"+index+"_lng").val()),place_id=$("#gmb_markers_group_"+index+"_place_id").val(),position=new google.maps.LatLng(marker_lat,marker_lng),marker_args={position:position,map:map,zIndex:index,icon:marker_icon,custom_label:marker_label};
//Is sign in enabled? And, do we have a place ID for this marker location?
place_id&&"enabled"===gmb_data.signed_in_option&&(
//Remove unnecessary array params
delete marker_args.position,
//Add Proper Params
marker_args.place={location:{lat:marker_lat,lng:marker_lng},placeId:place_id},marker_args.attribution={source:gmb_data.site_name,webUrl:gmb_data.site_url});
//Marker for map
var location_marker=new Marker(marker_args);markers.push(location_marker),location_marker.setVisible(!0),
//Set click action for marker to open infowindow
google.maps.event.addListener(location_marker,"click",function(){get_info_window_content(index,location_marker)}),time+=500}),cluster_markers===!0)var markerCluster=new MarkerClusterer(map,markers)}function get_info_window_saved_data(e){var a={};return a.title=$("#gmb_markers_group_"+e+"_title").val(),a.desc=$("#gmb_markers_group_"+e+"_description").val(),a.reference=$("#gmb_markers_group_"+e+"_reference").val(),a.place_id=$("#gmb_markers_group_"+e+"_place_id").val(),a.lat=$("#gmb_markers_group_"+e+"_lat").val(),a.lng=$("#gmb_markers_group_"+e+"_lng").val(),a}/**
...
info_bubble.close(),a.setVisible(!1)})}/**
	 ...
	 * @returns {Number}
	 */
function get_marker_index(){var e=$("#gmb_markers_group_repeat"),a=e.find(" div.cmb-repeatable-grouping"),t=e.find(".cmb-add-group-row.button"),o=parseInt(a.last().attr("data-iterator")),i=a.first().find("input,textarea").val();
//Ensure appropriate index is used for marker
//recount rows
return i&&0===o?(t.trigger("click"),o=1):0!==o&&(t.trigger("click"),o=parseInt(e.find(" div.cmb-repeatable-grouping").last().attr("data-iterator"))),o}/**
...
for(location_marker_array.length=0,e=0;e<info_bubble_array.length;e++)info_bubble_array[e].close(),google.maps.event.trigger(info_bubble_array[e],"closeclick");info_bubble_array.length=0}/**
	 ...
	 */
function toggle_marker_animation(){clear_main_markers()}/**
...
function set_street_view(){var e=$("#gmb_street_view").val();map.setOptions("none"===e?{streetViewControl:!1}:{streetViewControl:!0})}/**
	 ...
	 */
function set_map_type(e){e===!0&&$("#gmb_theme").val("none");var a=$("#gmb_type").val().toUpperCase();map.setOptions({mapTypeId:google.maps.MapTypeId[a],styles:!1})}/**
	 * Sets the Map Theme
	 *
	 * Uses Snazzy Maps JSON arrow to set the colors for the map
	 */
function set_map_theme(reset){var preset_theme=$("#gmb_theme"),map_theme_input_val=parseInt(preset_theme.val()),custom_theme_json=$("#gmb_theme_json");reset===!0&&($("#gmb_type").val("RoadMap"),$(".cmb2-id-gmb-theme-json").hide()),$(".custom-snazzy-toggle").on("click",function(){preset_theme.val("custom"),$(".cmb2-id-gmb-theme-json").show(),custom_theme_json.focus(),set_custom_snazzy_map()}),
//On Custom Map value change
custom_theme_json.on("change",function(){set_custom_snazzy_map()}),
//Snazzy maps set to none
"none"===preset_theme.val()?(set_map_type(),$(".cmb2-id-gmb-theme-json").hide()):"custom"===preset_theme.val()?($(".cmb2-id-gmb-theme-json").show(),custom_theme_json.focus(),set_custom_snazzy_map()):
//AJAX to get JSON data for Snazzy
$.getJSON(gmb_data.snazzy,function(data){$.each(data,function(index){data[index].id===map_theme_input_val&&(map_theme_input_val=eval(data[index].json))}),map.setOptions({mapTypeId:google.maps.MapTypeId.ROADMAP,styles:map_theme_input_val})})}/**
	 * Custom Snazzy Maps
	 */
function set_custom_snazzy_map(){var custom_theme_json=$("#gmb_theme_json");try{var custom_theme_json_val=$.parseJSON(custom_theme_json.val());map.setOptions({mapTypeId:google.maps.MapTypeId.ROADMAP,styles:eval(custom_theme_json_val)})}catch(err){alert("Invalid JSON"),custom_theme_json.val("").focus()}}/**
...
//SVG/Font icon Click
$(".icon").on("click",function(){$(".icon").removeClass("marker-item-selected"),$(this).addClass("marker-item-selected"),$(".save-marker-icon, .marker-label-color-wrap").show(),//slide down save button
...
//click to add marker
$(".drop-marker").on("click",function(e){if(e.preventDefault(),$(this).hasClass("active"))$(this).html(gmb_data.i18n.btn_drop_marker).removeClass("active"),map.setOptions({draggableCursor:null});else{$(this).text(gmb_data.i18n.btn_drop_marker_click).addClass("active"),map.setOptions({draggableCursor:"crosshair"});var a=google.maps.event.addListener(map,"click",function(e){drop_marker(e.latLng,a)})}});
//Radius Fields
var e;
...
//Marker Animation
$("#gmb_marker_animate1").change(function(){toggle_marker_animation()}),
//Close repeaters
$(".cmb-repeatable-grouping").addClass("closed"),
//Add Repeater toggle button
$(".toggle-repeater-groups").on("click",function(e){e.preventDefault(),$("#gmb_markers_group_repeat").find(".cmb-repeatable-grouping").toggleClass("closed")}),
//Window resize
